...
class ApplicationController extends Controller {
	...
	public function actionMain() {
		...
		if ($status == 'TRUE')
		{
			...
			if($id === null) {
				...
				$alert = 1;
				$alert_text .= 'We’re getting you started with a $10,000 beginner’s bonus, so go ahead…';
			}
			else {
				...
				if(($today-$last_visit)==0) {
					// do nothing
						...
						$id->save();
						$alert = 1;
						$alert_text .= '<br>Welcome back! Here’s your daily bonus of R$500. Start Predicting…<br>';
	...
	public function actionCreateTransactions() {
		...
		$criteria->condition = 'status="closed" AND pot_distributed="yes" AND correct_option<>"undefined"';
	...
	public function actionCreateTransactions2() {
		$criteria = new CDbCriteria;
		$criteria->select = 'id, correct_option';
		$criteria->condition = 'status="closed" AND pot_distributed="yes" AND correct_option<>"undefined"';
    	$questions = Questions::model()->findAll($criteria);
		foreach($questions as $id=>$question){
    		$this->getBets2($question->correct_option, $question->id);
		}
	}
	
	public function getBets2($correct_option, $question_id) {
		date_default_timezone_set('Asia/Calcutta');
		if($correct_option == 'cancelled') {
			$criteria = new CDbCriteria;
			$criteria->select = 'user_id, bet_amount, event_id';
