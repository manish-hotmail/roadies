<?php	
	...
	foreach ($EventModel as $key => $value) {
		...
		$event_info = "<table>";
			$event_info .= "<tr>";
				$event_info .= "<td style='width: 40%;'><h4 class='home_headers'><span class='header_text'>EVENT :</span><span>$value->title</span></h4></td>";
				$event_info .= "<td style='width: 35%;'><h4 class='home_headers'><span class='header_text'>TOTAL PREDICTIONS :</span><span id='event_total_bets'>$value->total_bets</span></h4></td>";
				$event_info .= "<td style='width: 25%;'><h4 class='total_pot'>TOTAL POT <br><span>R$ </span><span id='event_total_pot'>$value->total_pot</span></h4></td>";
			$event_info .= "</tr>";
...
<?php
	...
	$this->breadcrumbs=array(
		'Latest Events'=>array('/application/latestEvents'),
		$event_name	
	);
?>
<div id="bet_popup" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	  ...
	  <div class="modal-body">	   
		...
		<p style="margin: 0px; font-size: 20px; font-weight: 700;">R$ <input id="bid_amount" type="text" /></p>	    
		<div id="balance_meta_data">
	    	<p>Available Balance: R$ <span id="userAmount"></span></p>
	    	<p id="max_allowed_meta_data">(Maximum allowed R$ <span id="userQuestionPotAvailableAmount"></span>)</p>
	    </div>
...
<input type="text" id="xyz" style="width: 1px; display: none; height: 1px; position: absolute; top: 0px;" />
...
<script>
	...
	function populateBet(count, id, question_id, question, option, max_bid_amount){
		//alert(id);
		//window.scrollTo(0);	
		document.getElementById('bet_popup').style.top=count*10+10+'%';
	...
	function saveBet(){		
		...
		if (bid_amount <= 0 || isNaN(bid_amount)){
			...
			document.getElementById('error_msg').innerHTML = "You do not have enough Balance. Your R$ Balance is: "+uP;
			document.getElementById('bet_model_footer').style.display = "block";
		}			
	...
	var inter = setInterval(function() {
		...
		var msg = '';
	    var nm = '<?php echo $user_name ?>' + ' just placed R$ ' + bid_amount + ' on '+ option_text_global + '.';  
	    var ln = 'apps.facebook.com/roadiesprediction/';
	    var cap =  'Think you can predict what happens on Roadies? Join the MTV Roadies Prediction Game and win lots of prizes!';
	    var desc = question_text_global;
	    var pic = 'https://appstest.likemyworld.com/roadiesx/images/Roadies_200.jpg';
