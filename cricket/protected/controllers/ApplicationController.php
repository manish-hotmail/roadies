...
class ApplicationController extends Controller {
	...
	public function actionMain() {
		...
		if ($status == 'TRUE')
		{
			...
			$likes = $authentic->getFbPageLikes(89743390101, $access_token);
			$fb_me = $authentic->getMe();	
			$criteria=new CDbCriteria;
			$criteria->condition='uid=:uid';
			$criteria->params=array(':uid'=>$fb_me['id']);
            header('P3P:CP="IDC DSP COR ADM DEVi TAIi PSA PSD IVAi IVDi CONi HIS OUR IND CNT"');
            Yii::app()->request->cookies['uid'] = new CHttpCookie('uid', $fb_me['id']);
			...
			if($id === null) {
				...
				$alert = 1;
				$alert_text .= 'Here’s a shot at fame and fortune for all you cricket buffs out there! MTV presents Cricket Predictions, the only way to enjoy the game more than the guys playing it. Predict daily match events this T20 season, show us you know where to put your money and win cool prizes & cash! <br>We’re getting you started with a C$10,000 beginner’s bonus, so go ahead…<Start Predicting>';
			}
			else {
				...
				if(($today-$last_visit)==0) {
					// do nothing
						...
						$id->save();
						$alert = 1;
						$alert_text .= '<br>Welcome back! Here’s your daily bonus of ¢500. Start Predicting…<br>';
			...
			if($bonus_status != 'liked') {
					...
					$alert = 1;
					$alert_text .= '<br>Hey, You have just grabbed your MTV India Page Like Bonus of '. $bonus_amount .'. Good going!<br>';
					$user->save();
				}
			}
    ...
    public function actionWinners() {
        ...
        $criteria_e->select = 'id, title';
        $criteria_e->order = 'end_time DESC';
        $events = Events::model()->findAll($criteria_e);
       // print_r($events);
        $event_toppers_array = array();
        foreach ($events as $key => $event) {
            $event_toppers = array();
            $event_toppers['data'] = array();
            $command = "select user_id, sum(amount) as sum_amount from transaction where (transaction_type = 5 or transaction_type = 4) and user_id in (select id from user_info where block_status = 'unblock') and event_id =". $event->id ." group by user_id order by sum(amount) desc limit 1";
            $toppers = Yii::app()->db->createCommand($command)->queryAll();
            //print_r($toppers);
            $count = 1;
            $event_id = $event->id;
            ...
            foreach ($toppers as $key => $value) {
                ...
                $toppers_array1['rank'] = $count;
                $count++;
                array_push($event_toppers['data'], $toppers_array1);
                //print_r($event_toppers_array);
            }
            $event_toppers['event_id'] = $event->id;
            $event_toppers['event_title'] = $event->title;
           array_push($event_toppers_array, $event_toppers);
           // print_r($event_toppers_array);
           // print_r($event_toppers_array_js);
        }
        ...
        //$event_toppers_array_js = json_encode($event_toppers_array);
       // print_r($event_toppers_array_js);
		$this->render('leaderboard_new', array('response'=> $event_toppers_array, 'uid'=> $uid, 'my_info'=>$lb_data_users1));
    }
	...
	public function actionGiveAirtelAdvantageBonus() {
		...
		$access_token = $facebook->getAccessToken();
		if ($status == 'TRUE') {
			$likes = $authentic->getFbPageLikes(89743390101);
			$fb_me = $authentic->getMe();			
			$me = $fb_me['id'];
			$criteria = new CDbCriteria;
			$criteria->condition = 'uid=:uid';
	...
	public function actionCreateTransactions() {
		$criteria = new CDbCriteria;
