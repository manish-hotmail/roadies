<script>
    function gotoCurrentEvents () {
        window.location = "<?php echo $this->createUrl('/application/latestEvents'); ?>";
    }
</script>

<div id="roadies_intro">
	<h4>About Cricket Predictions</h4>
	<p>
		Predict daily match events this T20 season and win cool prizes and cash!
	</p>
</div>	
<table id="main_page_table" >	
	<tr>
		...
		<td class="sidebar_td" >
			<p class="main_page_headers"><span>UPCOMING MATCHES</span></p>
		...
		$.ajax({
			...
			success: function(response) {
				...
				var event_id;
						...
						$.each(d[key], function (index, element) {
							if (index == "id"){
								event_id = element;
	                  			...
	                  			html += '<td><div class="total_pot"><h4>TOTAL POT</h4><h3>C$ '+element+'</h3><button onclick="gotoEvent('+ event_id +')" class="btn-danger" style="font-size: 12px;">PREDICT NOW</button></div></td></tr></table></tr>';
	                  		} else if (index == "start_time") {
                    ...
                    html += '<tr><td><span style="cursor: pointer; float: right; color: #000000; margin-right: 30px; margin-top: 10px; "  onclick="gotoCurrentEvents()" >More>></span><td></tr>';
					html += '</table>';
