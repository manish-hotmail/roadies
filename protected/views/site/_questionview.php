	...
	foreach ($options as $key => $value) {
		$apH .= "<tr>";
			...
			if($status == "current"){
				...
				$apH .= "<button class='btn-danger' onclick='populateBet(".$value['id'].",".$question_id.",\"".CHtml::encode($question_text)."\",\"".CHtml::encode($value['option_text'])."\",".$question_max_bid_amount.")'>PREDICT</button>";
				$apH .= "</td>";
			} else {
